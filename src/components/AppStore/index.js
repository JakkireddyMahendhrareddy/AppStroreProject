...
class AppStore extends Component {
  ...
  appsResults = searchResults => {
    const {activeTab} = this.state
    const result = searchResults.filter(each => each.category === activeTab)
  ...
  render() {
    ...
    const searchResults = this.inputResults()
    const finalResults = this.appsResults(searchResults)
      ...
      <div className="container">
        <div className="inner-container">
          ...
          <input
            type="text"
            className="input"
            onChange={this.searchInputResult}
            value={searchInput}
          />
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                tabDetails={eachTab}
                getTabId={this.tabIdResults}
              />
            ))}
          </ul>
          <ul className="app-container">
            {finalResults.map(eachApp => (
